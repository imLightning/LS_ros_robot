...
def navToPoseOnce(navigator, pose, overtime):
    # 设置目标点坐标
    goal_pose = PoseStamped()
    goal_pose.header.frame_id = 'map'
    goal_pose.header.stamp = navigator.get_clock().now().to_msg()
    goal_pose.pose.position.x = pose["x"] * 1.0
    goal_pose.pose.position.y = pose["y"] * 1.0
    ...
    while not navigator.isTaskComplete():
        ...
        # 超时自动取消
        if Duration.from_msg(feedback.navigation_time) > Duration(seconds=overtime):
...
if __name__ == '__main__':
    navToPoseOnce({"x": 1.5, "y": 0.0, "w": 0.0}, 600)